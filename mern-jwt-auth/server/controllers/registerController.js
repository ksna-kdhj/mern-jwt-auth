...
const handleNewUser= async (req,res) =>{
    ...
    if(check)
    return res.status(409).json({'message':'provided username already exists'})
    ...
    if (!result) return res.status(400).json({'message':'Enter Valid Roles'})
    }
    try{
        const hashPwd = await bcrypt.hash(pwd, 10)
        await new User({username:user,roles:{...req.body.roles,'User':2001},password:hashPwd}).save()
